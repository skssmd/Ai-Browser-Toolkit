...
def test_unknown_op_is_rejected_before_the_browser(client):
    body = client.post("/command-list", json={"op": "teleport"}).json()
    assert body["error"]["type"] == "invalid_op"


def test_malformed_json_is_a_400(client):
    response = client.post(
        "/command-list", content=b"{not json", headers={"content-type": "application/json"}
    )
...
def test_batch_body_must_be_a_list(client):
    response = client.post("/command-list", json={"commands": "nope"})
    assert response.status_code == 400
    assert response.json()["error"]["type"] == "invalid_op"
...
def test_run_js_returns_a_value(client):
    body = client.post(
        "/command-list", json={"op": "run_js", "script": "return arguments[0] * 2;", "args": [21]}
    ).json()
