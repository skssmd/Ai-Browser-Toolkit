...
def run_episode(
    ...
    # call, so the budget has to leave room for that: the window is 32000.
    #
    # OpenRouter checks affordability against max_tokens rather than usage, so
    # a thin balance refuses the request outright -- "you requested up to
    # 32000 tokens, but can only afford 22583" -- without a token being spent.
    # That is a balance to top up, not a reason to shrink the window.
